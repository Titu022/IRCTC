...
const withIdempotency = async(key, fn) => {
    const existing = await prisma.idempotecyRecord.findMany({
        where: {eventKey: key}
    });
    ...
    await prisma.idempotecyRecord.create({
        data:{
            eventKey: key,
            response: result
        }
...
const createPaymentOrder = async(bookingId, amount, userId, idempotencyKey) => {
    if(!bookingId || !amount || !userId || !idempotencyKey){
        throw new BadRequestError('bookingId, amount, userId, idempotencyKey are required');
    }
    ...
    return withIdempotency(`payment-order:${idempotencyKey}`, async() => {
        ...
        const  gatewayResult = await gateway.createOrder(amount, 'INR', bookingId, {bookingId, userId});
        ...
        const paymentOrder = await prisma.paymentOrder.create({
            data:{
                ...
                status: 'CREATED',
                idempotencyKey,
                gatewayProvider: process.env.PAYMENT_GATEWAY,
        ...
        logger.info(`payment order created ${paymentOrder.id}`, {bookingId,
            gatewayOrderId: gatewayResult.gatewayOrderId
        });
...
const verifyAndCapturePayment = async(paymentOrderId, gatewayPaymentId, gatewaySignature) => {
    ...
    //verify signature
    const isValid = gateway.verifyPaymentSignature(paymentOrder.gatewayOrderId, gatewayPaymentId, gatewaySignature);
    ...
    if(!isValid){ 
        ...
        await paymentProducer.publishPaymentFailed(
            ...
            logger.error('Failed to publish PAYMENT_FAILED after signature verification failure', {error: err.message});
        })

        throw new BadRequestError('Payment signature verifiction failed', 'Invalid signature');
    ...
    logger.info(`Payment captured via verify`);
    ...
    await paymentProducer.publishPaymentSuccess(
        ...
        gatewayPaymentId,
        paymentOrder.amount
    ).catch(err => logger.error(`Failed to publish payment success after verify`, {error: err.message}));
