...
function startBookingExpiryJob() {
     ...
     expiryInterval = setInterval(cleanExpiredBookings, process.env.BOOKING_EXPIRY_CHECK_INTERVAL_MS);
     logger.info(
          `Booking expiry job started (interval: ${process.env.BOOKING_EXPIRY_CHECK_INTERVAL_MS}ms)`
     );
}
